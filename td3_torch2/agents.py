...
class Agent:
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer(
            self.memory_size, self.state_dims, self.action_dims
        )
    ...
    def load_batch(self):
        states, actions, rewards, states_, done = self.replay_buffer.load_batch(
            self.batch_size
        )
    ...
    def update_parameters(self, tau=None):
        ...
        for item in actor_state_dict:
            actor_state_dict[item] = (
                tau * actor_state_dict[item].clone()
                + (1 - tau) * actor_target_state_dict[item].clone()
            )
        for item in critic1_state_dict:
            critic1_state_dict[item] = (
                tau * critic1_state_dict[item].clone()
                + (1 - tau) * critic1_target_state_dict[item].clone()
            )
        for item in critic2_state_dict:
            critic2_state_dict[item] = (
                tau * critic2_state_dict[item].clone()
    ...
    def learn(self):
        ...
        noise = np.random.normal(
            0, self.noise_std, (self.batch_size, *self.action_dims)
        )
        noise = T.clamp(
            T.tensor(noise, dtype=T.float), -self.noise_clip, self.noise_clip
        ).to(self.actor.device)
        actions_ = self.target_actor.forward(states_).to(self.actor.device)
        actions_ = T.clamp(T.add(actions_, noise), self.min_action, self.max_action).to(
            self.actor.device
        )
        ...
        if self.learn_iter % self.update_period == 0:
            ...
            actor_loss = -T.mean(
                self.critic_1.forward(states, self.actor.forward(states))
            )
            actor_loss.backward()
